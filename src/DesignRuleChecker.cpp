...
int DesignRuleChecker::checkAcuteAngleViolationBetweenTracesAndPads() {
    ...
    std::cout << std::fixed << std::setprecision((int)round(std::log(INPUT_PRECISION)));
        ...
        for (auto &pin : pins) {
            ...
            for (auto &seg : net.getSegments()) {
                ...
                if (bg::crosses(bgLs, exactLocGridPadShapePoly)) {
                    Point_2D<long long> pt1{(long long)round(points[0].x() * INPUT_PRECISION), (long long)round(points[0].y() * INPUT_PRECISION)};
                    Point_2D<long long> pt2{(long long)round(points[1].x() * INPUT_PRECISION), (long long)round(points[1].y() * INPUT_PRECISION)};
                    if (pad.getPadShape() == padShape::RECT) {
                        if (!isOrthogonalSegment(pt1, pt2)) {
                            ...
                            if (!intersectPts.empty()) {
                                if (!isIntersectionPointOnTheBoxCorner(intersectPts.front(), exactLocGridPadShapePoly, seg.getWidth())) {
...
bool DesignRuleChecker::isIntersectionPointOnTheBoxCorner(point_double_t &point, polygon_double_t &poly, double wireWidth) {
